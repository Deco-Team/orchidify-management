...
import { ReportCourseByMonthListItemResponseDto } from '~/data/reportAdmin.dto'
import { useStatisticApi } from '~/hooks/api/useStatisticApi'
import { notifyError } from '~/utils/toastify'

const CourseDataChart = () => {
  const [selectedYear, setSelectedYear] = useState<number>(2024)
  const { getReportCourseByMonth } = useStatisticApi()
  const [chartData, setChartData] = useState<ListResponseDto<ReportCourseByMonthListItemResponseDto>>({
    docs: [],
    totalDocs: 0,
    offset: 0,
    limit: 0,
  ...
  useEffect(() => {
    ;(async () => {
      const { data: reportData, error: apiError } = await getReportCourseByMonth(selectedYear)
      if (reportData) {
  ...
  }, [getReportCourseByMonth, selectedYear])
...
interface ChartDisplayProps {
  data: ReportCourseByMonthListItemResponseDto[]
}
...
const ChartDisplay = ({ data }: ChartDisplayProps) => {
  const chartSeries: ApexCharts.ApexOptions['series'] = useMemo(() => {
    ...
    data.forEach((month) => {
      course.push(month.course.quantity)
    })
